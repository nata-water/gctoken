...
function calculateAiCredits(costUsd: number): number {
...
async function getCopilotSessionFiles(
  lookbackDays: number,
...
export async function scanUsage(lookbackDays: number): Promise<UsageResult> {
  const { files: sessionFiles, paths: scannedPaths } =
    await getCopilotSessionFiles(lookbackDays);
  ...
  for (const filePath of sessionFiles) {
    ...
    const estimatedAiCredits =
      parsed.aiCredits > 0 ? parsed.aiCredits : calculateAiCredits(calculatedCost);
    const estimatedCost =
      parsed.aiCredits > 0 ? parsed.aiCredits * AI_CREDIT_USD : calculatedCost;
