...
import type { PeriodStats, UsageResult } from "./types.js";
...
function getPeriodStats(
  result: UsageResult,
  period: CliOptions["period"],
): { label: string; stats: PeriodStats } {
  switch (period) {
    case "today":
...
function printText(result: UsageResult, opts: CliOptions): void {
  ...
  if (stats.thinkingTokens > 0) {
    lines.push(
      `  Thinking:   ${formatNumber(stats.thinkingTokens).padStart(12)}`,
    );
  }
  ...
  if (opts.models && Object.keys(stats.modelUsage).length > 0) {
    ...
    lines.push(
      `${"  Model".padEnd(32)} ${"Input".padStart(10)} ${"Output".padStart(10)}`,
    );
    lines.push(`  ${"─".repeat(50)}`);

    const sorted = Object.entries(stats.modelUsage).sort(
      ([, a], [, b]) =>
    ...
    for (const [model, usage] of sorted) {
      lines.push(
        `  ${model.padEnd(30)} ${formatNumber(usage.inputTokens).padStart(10)} ${formatNumber(usage.outputTokens).padStart(10)}`,
      );
    }
  }
...
function printJson(result: UsageResult, opts: CliOptions): void {
  const { label, stats } = getPeriodStats(result, opts.period);
